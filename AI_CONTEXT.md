...
- **阶段**: STM32 固件端**Jetson联调阶段**已完成里程计数据打通与方向测试。
- **已完成**: CAN 双向通信 ✅ | 自愈重连机制 ✅ | USART3 串口接收引擎 ✅ | 实车热插拔测试 ✅ | **Jetson 下发控制测试** ✅ | **里程计高速上报测试** ✅
- **当前测试方式**: STM32 与底盘 CAN 正常通信，并在 USART3 的 TX 引脚上按照 50Hz (20ms) 自动上发 11 字节的里程计定制帧。Jetson 端下发控制代码已完成，正在准备接收。
- **下一步**: 开发 Jetson 端的 ROS2 里程计接收处理节点，执行 `/odom` 话题发布；并着手处理后续的电池状态上报。
...
- **XOR 校验**: 对 `AA 55 01 + 6字节数据` 共计 9 个字节进行异或。串口调试助手已实测证明其极为稳定。
...
死循环:
  ...
  if 收到数据:
    ...
    0x104 到了: 调用 jetson_report_odom(vx_fb, vy_fb, vz_fb)
               → 通过 USART3 TX 以 [AA 55 01 ...] 11字节帧上报 Jetson (50Hz)
  无论是否收到数据:
...
### 任务一：Jetson ROS2 控制联调（核心目标）[已完成一半]
...
- ⏳ **待完成**: 在 Jetson 端 ROS2 串口节点中，增加对 `AA 55 01` 的滑窗搜帧截获，执行 `(Vx*cos...)*dt` 积分，并向外发布 `/odom` 与 TF。

### 任务三：状态监控上报
- ⏳ 暂未开始。计划将 0x100 中的 SOC、电压、电流、故障码，以 `TYPE = 0x02` 的 11 字节新数据包格式低频上报给 Jetson。
- 目标：在 RViz 或监控节点显示电池状态和故障预警。
